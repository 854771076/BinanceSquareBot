...
class BinanceAnnCliService:
    """CLI workflow: crawl Binance official announcements, polish, publish."""

    SOURCE_NAME = "BinanceAnnSource"

    def __init__(self, dry_run: bool = False, limit: int | None = None) -> None:
        self.dry_run = dry_run
        self.limit = limit
        self.storage = StorageService()
        self.source = BinanceAnnSource()  # type: ignore[no-untyped-call]
        self.target = BinanceTarget()  # type: ignore[no-untyped-call]
        self.publisher = AccountItemPublisher()

    def execute(self) -> dict[str, Any]:
        logger.info("Starting BinanceAnn workflow")
        if not self.source.config.enabled:
            console.print(
                "[yellow]BinanceAnnSource is disabled "
                "(set BINANCE_ANN_SOURCE_ENABLED=true).[/yellow]"
            )
            return self._empty_stats()
        if not self.storage.can_execute_source(
            self.SOURCE_NAME, self.source.config.daily_max_executions
        ):
            console.print("[yellow]⚠️ Daily limit reached for BinanceAnnSource[/yellow]")
            return {**self._empty_stats(), "error": "daily limit reached"}

        console.print("[blue]📥 Fetching Binance announcements...[/blue]")
        items = self.source.fetch()
        console.print(f"✓ Prepared {len(items)} announcement items")
    ...
    def _print_summary(self, stats: dict[str, Any]) -> None:
        table = Table(title="BinanceAnn Workflow Summary")
